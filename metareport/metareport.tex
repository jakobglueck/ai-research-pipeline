...
durch. Ob sie damit auch den vollständigen wissenschaftlichen Erkenntnisprozess autonom
bestreiten können, von der ersten Datenexploration bis zum fertigen Paper, ist bislang
offen.
...
\cite{schmidgall2025agentlab}. Der Unterschied des vorliegenden Ansatzes liegt nicht
in der maximalen Automatisierung, sondern in einer strukturierten, architektonisch
verankerten Qualitätssicherung.

Die Baseline-Untersuchung offenbarte fundamentale Limitationen unstrukturierter Prompts.
Ohne Echtzeitzugriff auf externe Werkzeuge neigen Modelle zu Halluzinationen bei
Literaturangaben \cite{rao2026detecting}. Zudem belegt die Literatur, dass LLMs bei
der Selbstüberprüfung zwischen 52 bis 82\,\% ihrer eigenen methodischen Fehler
übersehen \cite{badscientist2025} und dazu tendieren, statistische Signifikanz mit
praktischer Relevanz gleichzusetzen \cite{badscientist2025}.

Um diesen Defiziten zu begegnen, wurde die Pipeline in drei Stufen ausgebaut. Qualität
wurde dabei operationalisiert als die Erfüllung verifizierbarer Kriterien: valide
Zitationen, methodische Robustheit, Schutz vor HARKing und Reproduzierbarkeit der
Ergebnisse. Forschungsfrage und Hypothesen lauteten:
...
\begin{description}
  ...
  \item[$H_0$:] Eine strukturierte Pipeline erzeugt keinen messbaren Qualitätsunterschied.
  \item[$H_1$:] Eine strukturierte Pipeline verbessert die Qualität in mindestens einer Dimension.
\end{description}
...
die semantische und syntaktische Strukturen modellieren \cite{brown2020gpt3}. Ihre
Steuerung erfolgt über Prompts; die Generierung basiert auf statistischen Mustern
der Trainingsdaten. Für den wissenschaftlichen Einsatz entscheidend: Moderne Modelle
verfassen nicht nur natürliche Sprache, sondern erzeugen auch Programmcode sowie
strukturierte Datenformate.

\subsection{Autonome Agenten und Pipelines}

Ein Chatbot reagiert auf Eingaben und antwortet. Ein autonomer Agent geht weiter:
Er erhält ein Ziel und entscheidet eigenständig, welche Schritte dorthin führen
\cite{lu2024aiscientist, schmidgall2025agentlab}. Konkret in diesem Projekt: Der
Agent erhält einen Datensatz und liefert am Ende ein Paper ab, ohne menschliche
...
Das Model Context Protocol (MCP) \cite{anthropic2025mcp} ist ein offenes Protokoll,
das Sprachmodellen standardisierten Zugriff auf externe Werkzeuge verschafft. Über
MCP-Server kann ein Modell zur Laufzeit Datenbanken abfragen, Webseiten abrufen oder
Dateien bearbeiten, statt allein auf sein Trainingswissen angewiesen zu sein. Zwei
...
Unter der Baseline-Bedingung formulierte der Agent aus einem einzigen Prompt heraus
eine Forschungsfrage, führte statistische Analysen in Python durch und erstellte ein
vollständiges Paper. Ohne Werkzeugzugriff traten gravierende Defizite auf:
Literaturangaben konnten nicht in Echtzeit verifiziert werden, die Herkunft
berechneter Werte aus dem Datensatz ließ sich nicht nachvollziehen, und die
Korrektheit von Seitenzahlen konnte nicht kontrolliert werden. Diese Defizite
...
Datenbankzugriff und Fetch-MCP zum Echtzeit-Abruf von Literaturquellen. MCP
\cite{anthropic2025mcp} erwies sich gegenüber reinen Websuchanfragen als überlegen,
da der Agent Abstracts inhaltlich validieren und deren Relevanz dokumentieren musste,
bevor eine Quelle zitiert werden durfte.
...
Besonders aufschlussreich war die sogenannte "`Likert-Rebellion"': Der Agent
ignorierte eine explizite Dummy-Coding-Vorgabe und behandelte die Variablen
stattdessen ordinal. Seine Begründung war statistisch: Dummy-Coding ignoriere die
Rangordnung der Likert-Kategorien und verschwende damit Ordinalskalierungsinformation.
Dies verdeutlicht, dass leistungsfähige Modelle Textanweisungen gegen ihr
inhärentes methodisches Training abwägen. Methodische Vorgaben sollten daher
bevorzugt als Code-Assertions implementiert werden und nicht ausschließlich
als deklarative Prompt-Anweisungen.
...
Eine feindliche statt neutraler Gutachterrolle wurde gewählt, weil LLMs bei
Selbstprüfung dazu neigen, eigene Fehler zu übersehen \cite{badscientist2025}.
...
Jahresgehalt. Bivariat lag der Effekt bei $d = -0{,}36$. Bemerkenswert: 81{,}6\,\% derjenigen, die KI als Bedrohung wahrnahmen, nutzten
KI-Tools dennoch aktiv. Wahrgenommene Bedrohung und tatsächliche Nutzung schließen
...
einbezogen wurde). Das Ergebnis war
klar: AIThreat zeigte einen substanziellen Effekt auf Jobzufriedenheit
...
Pre-Registrierungsbedingungen. Allerdings: v2 und v3 griffen auf dieselbe
Ausgangsvariable wie v1 zurück. Die Konsistenz ist damit teilweise durch das Design bedingt und stellt keine
...
Bemerkenswert ist dabei die ungleichmäßige Verteilung dieser Verbesserung. Eine
...
Dabei zeigt sich ein klares Muster. Textuelle Vorgaben erweisen sich als deutlich weniger zuverlässig
als Mechanismen, die direkt im Ausführungscode verankert sind. Die Likert-Rebellion illustriert
das eindrücklich für Methodenvorgaben, aber auch Qualitätspflichten scheitern, sobald
sie nur auf dem Papier stehen, statt tatsächlich durchgesetzt zu werden.
Code-Assertions, JSON-Locks, Blocking-Bedingungen halten stand, wo bloße
Prompt-Anweisungen versagen.
...
Hinzu kommt: Die verglichenen Bedingungen unterscheiden sich nicht nur in der
Pipeline-Architektur, sondern auch in der jeweiligen Forschungsfrage. Komplexität
der Pipeline und Komplexität der Fragestellung lassen sich damit nicht sauber trennen,
